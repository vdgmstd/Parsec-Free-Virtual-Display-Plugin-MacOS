...
# Parse value from Cargo.toml (handles both [package] and [package.metadata.*] sections)
parse_toml() {
    local key=$1
    grep "^${key} *= *" "${CONFIG_FILE}" 2>/dev/null | head -1 | sed 's/.*= *"\([^"]*\)".*/\1/'
}
...
APP_DISPLAY_NAME=$(parse_toml "display_name")
